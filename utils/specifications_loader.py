...
@dataclass(slots=True)
class FfmpegcvSpecificationLoader:
    sequence_name: str
    cameras_idx: list[int]
    pose_model: int
    triangulation_method: str
    video_captures: list[ffmpegcv.ffmpeg_reader.FFmpegReaderNV]
    frame_count: int
    mosaic_structure: MosaicStructure
    pose_detectors: list[PoseDetection]
    pose_buffers: list[np.ndarray]
    ...
    def __init__(self, specs_path: str):
        ...
        self.video_captures = [
            ffmpegcv.VideoCaptureNV(
                directory + video, resize=(tile_res[1], tile_res[0])
            ) for video in videos
        ]  # ffmpegcv.ffmpeg_reader.FFmpegReaderNV
        # self.video_captures = [
        #     ffmpegcv.noblock(
        #         ffmpegcv.VideoCaptureNV, directory + video, resize=(tile_res[1], tile_res[0])
        #     ) for video in videos
        # ]  # ffmpegcv.ffmpeg_noblock.FFmpegReaderNoblock
        print(type(self.video_captures[0]))
        self.frame_count = int(len(self.video_captures[0]))
        self.mosaic_structure = MosaicStructure(number_of_feeds, tile_grid, tile_res)
        self.pose_detectors = [
            PoseDetection(img_res, model, visibility) for _ in videos
        ]
        self.pose_buffers = [np.empty(0)] * number_of_feeds
        self.pose_triangulator = PoseTriangulation(cameras_params)
        self.pose_visualizer = PoseVisualization(cameras_params, cameras_idx)
